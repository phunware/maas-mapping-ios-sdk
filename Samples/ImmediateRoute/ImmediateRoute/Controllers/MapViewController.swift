...
class MapViewController: UIViewController, SegmentedViewController {
    ...
    var lastUpdateLocation: PWIndoorLocation?
...
extension MapViewController: PWMapViewDelegate {
    ...
    func mapView(_ mapView: PWMapView!, locationManager: PWLocationManager!, didUpdateIndoorUserLocation userLocation: PWIndoorLocation!) {
        if lastUpdateLocation == nil {
            PWRoute.createRoute(from: userLocation, to: /*CurrentLocationToEndPoint*/, accessibility: false, excludedPoints: nil, completion: { [weak self] (route, error) in
                DispatchQueue.main.async {
                    if let route = route {
                        self?.dismiss(animated: true, completion: nil)
                        NotificationCenter.default.post(name: .startNavigatingRoute, object: route)
