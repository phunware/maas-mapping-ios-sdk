...
extension StandardManeuverViewModel: ManeuverViewModel {
    ...
    var attributedText: NSAttributedString {
        ...
        if maneuver.isLastManeuver {
            let arriveString = " " + NSLocalizedString("to arrive at your destination", comment: "")
            attributed.append(.init(string: arriveString, attributes: standardOptions.attributes))
        }
        
        // Place a period at the end, using the same attributes as the last attributed substring.
        var lastAttributes: [NSAttributedString.Key : Any]? = nil
        
        let attributedRange = NSRange(location: 0, length: attributed.length)
        attributed.enumerateAttributes(in: attributedRange, options: .reverse) { (attributes, range, stop) in
            lastAttributes = attributes
            stop.pointee = true
        }
        
        if let last = lastAttributes {
            let period = NSAttributedString(string: ".", attributes: last)
